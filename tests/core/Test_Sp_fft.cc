...
void testDwfMatrix_momentum(GridCartesian &GRID, GridRedBlackCartesian &RBGRID, SpFundamentalRepresentation::LatticeField &Umu, const std::vector<int>& seeds, const int vol, const Coordinate latt_size, GridSerialRNG sRNG, GridParallelRNG pRNG)
{
  ...
  ////////////////////////////////////////////////////
    ...
    SpDomainWallFermionD Ddwf(Umu, *FGrid, *FrbGrid, GRID, RBGRID, mass, M5);

    typedef typename SpDomainWallFermionD::FermionField SpLatticeFermionD;
    ...
    SpLatticeFermionD result5(FGrid);
    ...
    Ddwf.M(src5, tmp5);
    ref5 = tmp5;
    //std::cout << "aaa " << norm2(tmp5) << std::endl;
    FFT theFFT5(FGrid);
